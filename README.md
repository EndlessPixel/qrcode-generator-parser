...
- **生成二维码**：输入文本或上传文件，实时生成二维码，并自动验证生成结果。
- **下载图片**：生成的二维码可一键下载为 PNG 图片。
- **解析二维码**：上传二维码图片，自动识别并显示文本内容。
- **文件二维码**：支持解析包含文件内容的二维码，并还原下载原始文件。
- **调试日志**：内置日志面板，记录用户操作、网络状态、设备信息等，方便排查问题。支持颜色编码和详细的日志格式。
- **日志管理**：支持下载日志文件、一键复制日志内容（含多种兼容方案）。
- **右上角提示**：美观的非阻塞式提示框，替代传统的 alert 弹窗。
...
### 生成二维码
- 在“生成二维码”选项卡中输入文本，或点击“或上传文件”下方的按钮选择文件。
...
- 如果二维码包含文件内容，会显示“下载还原文件”按钮，点击可下载还原的原始文件。

### 调试日志
- 点击页面右上角的“查看调试”按钮，可打开调试面板。
- 面板中实时记录所有操作步骤、错误信息、设备环境等，支持颜色编码。
- 日志格式：`[日期 时间 级别] 日志内容`
